...
- [ ] **Audit and fix eagle wall positions in `drawEagleBase()`**: game.js `drawEagleBase()` (lines 1107–1114) draws 4 wall metatiles at `(ex-16,ey-16)`, `(ex,ey-16)`, `(ex-16,ey)`, `(ex,ey)` — these 4 positions cover the exact same 32×32 area as the eagle sprite, so walls are drawn UNDER the eagle, not AROUND it. In the real NES game, the eagle walls form a U-shape surrounding the 32×32 eagle area (left, top, right — bottom open). The walls are BG nametable tiles placed by the level loader and modified by the Shovel power-up ($C9BB/$C912). Disassemble $C9BB (ShovelWallFortify) to extract the exact nametable positions of the 12 wall tiles (likely 3 metatiles left + 4 above + 3 right + 2 below-sides, forming the classic U-shape at metatile grid positions around col 5–8, rows 11–12). Then fix `drawEagleBase()` to draw walls at the correct surrounding positions, OR remove the wall drawing entirely and ensure the level grid data already contains the correct wall tiles at those positions.

- [ ] **Verify eagle area in LEVEL_MAPS grid data**: Check whether the 13×13 level grid already contains brick tiles around the eagle position (metatile cols 4–8, rows 11–12). If yes, `drawField()` already renders those walls via `drawTile()`, and `drawEagleBase()` should NOT draw additional wall metatiles (only the eagle sprite). If the grid has EMPTY at those positions, then `drawEagleBase()` must draw the walls at the correct surrounding positions. Compare against `render_level.py` output and `render_frame.py --stage 1` reference PNG.
...
- [ ] **Replace all `fillText`/canvas font usage with CHR tile text rendering**: game.js uses `ctx.fillText()` with monospace canvas fonts in ~15 places: stage banner "STAGE XX" (line 1453), game over "GAME"/"OVER" (lines 1461–1462), retry prompt (line 1465), stage clear tally (lines 1480, 1517, 1525, 1527, 1539, 1541), HUD score digits, and controls hint (line 1599). The ROM renders ALL text via PPU nametable tile writes using CHR font tiles — ASCII-indexed BG-bank tiles where tile $41='A', $42='B', ..., $5A='Z', $30='0', ..., $39='9' (confirmed from `DrawSpriteString ($D8F7)` and `StrGAME ($D214)=$47,$41,$4D,$45`). Build a `drawNesText(str, x, y, palIdx)` helper that maps each ASCII character to its CHR tile index and calls `drawCHRTile(tileIdx, palIdx, x + col*8, y)` per character. Then replace all `fillText` call sites. For large magnified text (GAME/OVER — ROM uses `DrawBigSpriteTile ($D87E)` which renders each CHR tile as a 32×32 magnified block via 4×4 OAM sprites), implement a `drawBigText` variant that scales each tile 4×. Palette for HUD/score text is BG3 (palIdx 3); palette for GAME/OVER large text is also BG3. Verify tile indices for digits (0–9 → CHR $30–$39), letters (A–Z → CHR $41–$5A), space → CHR $20 or skip, and special characters (hyphen, colon, exclamation) against the CHR sheet.
