...
- [ ] **Extract both CHR bank pairs and visually compare**: Run `extract_tiles.py` for both bank pairs — output `chr_bank01.png` (current) and `chr_bank23.png` (alternate). Visually compare key tiles: $00 (mortar vs char), $0F (brick), $10 (steel), $D0-$DF (eagle), sprite bank $00-$1F (player tanks), $80-$97 (power-ups). Identify which bank has the correct Battle City tile graphics for standard gameplay.
