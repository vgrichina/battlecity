...
- [ ] **Fix shield rendering to use CHR tiles**: ROM `PlayerShieldDraw ($E330)` draws shield via `DrawTank ($DB0A)` with `$04=$02` (SP2 palette=palIdx 6) and alternating tiles every 2 frames: when `frameCount & 2 = 0` → tile=$29 (DrawTank: left-col $29, right-col $2B); when `frameCount & 2 = 2` → tile=$2D (left-col $2D, right-col $2F). In 8×16 mode: even-frame draws `[TL=$28,TR=$2A,BL=$29,BR=$2B]`, odd-frame draws `[TL=$2C,TR=$2E,BL=$2D,BR=$2F]` — all BG bank (pt1=true). game.js currently uses `fillRect(e.x-9, e.y-9, TANK_SZ+2, TANK_SZ+2, C.SHIELD)` solid yellow rectangle. **Fix**: when `chrOff` available and `e.shieldTimer > 0 && (frameCount >> 1) & 1`: replace fillRect with two `drawSprite16` calls selecting between the two tile sets based on `(frameCount >> 1) & 1`.

- [ ] **Implement trees z-ordering**: ROM `DrawEntityTile ($DACC-$DAD4)` checks if sprite position falls on BG tile $22 (tree); if so, ORs OAM attribute with `$6E` (set to $20 = priority-behind-BG during gameplay at `$D3DF: LDA #$20; STA $6E`). This makes entities appear BEHIND tree tiles — a classic Battle City mechanic. game.js draws entities after all BG tiles so entities appear on top of trees. **Fix**: in `render()`, after calling `drawEntity()` for all slots, do a second pass to re-draw only TREE tiles (`T.TREES`) over the entities. Implementation: add `drawTreesOverlay()` called after `drawPowerUp()` that loops the grid and re-calls `drawTile(col,row)` for any `grid[row][col] === T.TREES`.

- [ ] **Add HUD animated tank wiggle**: ROM `HUDTankAnimation ($C7F8)` applies 4-direction oscillation delta to tank display position: `HUDTankWiggleX ($D2C6)` = {0,−1,0,+1}, `HUDTankWiggleY ($D2CA)` = {−1,0,+1,0}, cycling via `$0107` every frame while `$0108 ≥ $0A`. game.js draws HUD tanks at fixed (104, py=105) with no wiggle. **Fix**: add `hudTankWiggleIdx` state variable (0–3, advance each frame); apply deltas `[0,-1,0,+1][idx]` to HUD tank X and `[-1,0,+1,0][idx]` to HUD tank Y before drawing in render().
...
- [ ] **Document NMI draw loop and replicate as `render_frame.py`**: Trace NMI handler at `$D300` — (1) OAM DMA: write $02 to $4014 → copies $0200–$02FF to PPU OAM (64 sprites × 4 bytes); (2) PPU nametable queue: flush `$0400–$07FF` tile shadow via `PPUQueueTiles ($D6D3)` writing through `$2006/$2007`; (3) palette update: if palette-flash active, write changed palette bytes to $3F00+; (4) enable rendering: write PPU_MASK `$1E` to `$2001`. Write `render_frame.py` that reads the OAM buffer + nametable shadow at a given game state and renders a frame image using the CHR tile data — useful for pixel-perfect comparison with emulator screenshots.
