...
- [ ] **Implement Construction Mode**. **Open.** ROM $8B22 (Stage0InitPhase0) setup for tile editing. Need to add grid-edit UI and "SAVE" mechanism to custom Stage 0 slot.
- [ ] **Implement Tally Screen "Curtain" Reveal**. **Open.** ROM $CAAD/$CACF uses attribute buffer tricks to reveal/hide tally results. Web currently shows it as a static-transition overlay.
- [ ] **Improve Sound Engine Fidelity**. **Open.** Current `sound.js` is functional but lacks precise envelope/timing from `SoundSeqReadByte ($EE63)`. Need higher fidelity for specific SFX jingles.
- [ ] **Implement Credits/Coin workflow**. **Open.** ROM $C840 (CoinScreen) manages `Credits ($4B)`. Need to add "Press C to insert coin" logic to match VS. System arcade experience.
