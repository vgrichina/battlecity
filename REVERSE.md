...
| $0400–$07BF   | Nametable tile shadow (960 bytes, 32×30 tile indices; mirrors PPU nametable $2000–$27BF; rebuilt each frame by PPUQueueTiles $D6D3 shadow writes) |
| $07C0–$07FF   | Nametable attribute shadow (64 bytes; mirrors PPU $23C0–$23FF attribute table; palette index per 4×4-tile block) |
...
| $DFB1 | DrawSpawnSprite | Draw score popup sprite using $A8,X to index into tile block (draws score popup "100"/"200"/"300"/"400" CHR tiles $B8-$C5) |
...
| `$01`–`$0E` | Brick partial-damage states | `$D763` (bullet-hit handler writes 4-bit bitmask directly as collision-map value AND nametable CHR tile index) | Bitmask encodes which of the 4 quads (TL/TR/BL/BR) are still intact: bit 3=TL, bit 2=TR, bit 1=BL, bit 0=BR. Value $0F = all intact; $00 = fully destroyed (tile becomes $00). The same nibble simultaneously serves as the CHR tile index |
...
| `$15`–`$1F` | *Not found in code yet* | — | 17 tiles; CHR data present; no ROM code or data references located |
...
| `$23`–`$27` | *Found in data, purpose unclear* | `$D1CD` (PPUQueueTiles col 7 row 7 on stage-select screen at `$C107`); also `$D1DF` (col 13 row 16) | Appear in the stage-select BG nametable row alongside tile IDs `$9E`/`$9F`; exact visual purpose not yet identified |
...
| `$2A`–`$2B` | *Not found in code yet* | — | 2 tiles between shield frame A and B |
...
| `$2E`–`$2F` | *Not found in code yet* | — | 2 tiles after shield frame B, before digits |
| `$3A`–`$3B` | Score popup "500" — drawn by PowerUpCollision (`$EB17`) on power-up collection (`$62`=`$32` flash); OAM tile `$3B` (odd → PT1 `$3A`–`$3B`); 500-pt bonus displayed for 50 frames | `$E2EF` (DrawPowerUp: if `$62>0` draws flash sprite OAM tile `$3B` → PT1 tiles `$3A`/`$3B` left col + `$3C`/`$3D` right col); `$EB17` (PowerUpCollision: sets `$62`=50 on pickup) | 8×16 score-popup sprite at collect position; `$3B` odd → PT1; decrement timer at `$E2EF` until `$62`=0 clears effect |
| `$3C`–`$3F` | ASCII symbols `< = > ?` | `$D1CD` (stage-select BG row data); `$D1DF` (stage-select row 16 data) | Tile indices match ASCII values: `$3C`=`<`, `$3D`=`=`, `$3E`=`>`, `$3F`=`?`; appear as decoration/framing tiles in the stage-select display |
| `$40` | `©` copyright symbol | `$D1E5` (copyright string: `$40 20 31 39 38 30 ...` = `©1980 1985 NAMCO LTD`); `$C107` (PPUQueueTiles col 7 row 7) | ASCII `@` slot repurposed as the copyright glyph; drawn on stage-select/title screen |
| `$41`–`$5A` | ASCII uppercase A–Z | All text strings in `$D14F`–`$D2BA` (BATTLE, CITY, PLAYER, PUSH START BUTTON, PLEASE INSERT COIN, HISCORE, CREDIT, STAGE, PEACE BE WITH YOU, NOW LONG WAR, COMES TO, AN END, BONUS, TOTAL, etc.); drawn via `PPUQueueTiles ($D6D3)` and `DrawSpriteString ($D8F7)` | Tile index = ASCII value: `$41`=A … `$5A`=Z. Used for all on-screen text as both BG nametable tiles and OAM sprite tiles (with `$60` offset applied in DrawSpriteString) |
| `$5B` | `[` bracket | `$D271` (tally screen data array) | Used in end-of-level tally display |
| `$5C` | Dashed separator line tile | `$D27D` (tally screen: `[$5C×8,$FF]` = 8-tile horizontal row) | Drawn as a full-width separator bar in the end-of-level tally screen; exact visual glyph unclear (not a standard ASCII backslash) |
...
| `$60`–`$68` | NAMCO logo tiles | `$D145` (VictoryTileData: `[$60–$68,$FF]`), `$D04C` (stage-start col 13 row 23), `$C44D` (victory screen col 12 row 14) | 9 tiles form "NAMCO" word as thick block-letter graphic strip |
| `$69` | Decorative tail symbol | `$D1FC` (copyright string `©1980 1985 NAMCO LTD`+`$69`+`$FF`); `$D2BD` (victory string `AN END`+`$69`+`$FF`); drawn via PPUQueueTiles | Appears as a trailing decoration after "NAMCO LTD" and "AN END"; exact visual unclear — not found in code as a tile draw target, only in data strings |
...
| `$6E`–`$77` | *Found in data, purpose unclear* | `$D1CD` (stage-select BG row at col 7 row 7, `$C107`); `$C0CB` stores `$6E` into `$60` (tile-draw offset register) before stage-select display | 10 tiles; appear in the stage-select nametable display row. `$6E` also used as the DrawSpriteString tile-offset value (`STA $60`) — may indicate these tiles are alphabet letters shifted by `$6E`-`$41`=`$2D` from ASCII, i.e. alternate-style uppercase glyphs |
...
| `$82`–`$83` | *Not found in code yet* | — | 2 tiles between helmet and clock power-up icons |
| `$84`–`$85` | Power-up: clock (type 1) | `$E31C` (DrawPowerUp type=1: tile=`$85`; OAM `$85` → PT1 `$84`–`$85`) | Freeze/timer power-up icon |
| `$86`–`$87` | *Not found in code yet* | — | 2 tiles between clock and shovel power-up icons |
| `$88`–`$89` | Power-up: shovel (type 2) | `$E31C` (DrawPowerUp type=2: tile=`$89`; OAM `$89` → PT1 `$88`–`$89`) | Fortify-base (shovel) power-up icon |
| `$8A`–`$8B` | *Not found in code yet* | — | 2 tiles between shovel and star power-up icons |
| `$8C`–`$8D` | Power-up: star (type 3) | `$E31C` (DrawPowerUp type=3: tile=`$8D`; OAM `$8D` → PT1 `$8C`–`$8D`) | Upgrade-star power-up icon |
| `$8E`–`$8F` | *Not found in code yet* | — | 2 tiles between star and grenade power-up icons |
| `$90`–`$91` | Power-up: grenade (type 4) | `$E31C` (DrawPowerUp type=4: tile=`$91`; OAM `$91` → PT1 `$90`–`$91`) | Grenade (destroy all enemies) power-up icon |
| `$92`–`$93` | *Not found in code yet* | — | 2 tiles between grenade and pistol power-up icons |
| `$94`–`$95` | Power-up: pistol (type 5) | `$E31C` (DrawPowerUp type=5: tile=`$95`; OAM `$95` → PT1 `$94`–`$95`) | Fast-bullet power-up icon |
| `$96`–`$97` | *Not found in code yet* | — | 2 tiles between pistol and tank power-up icons |
| `$98`–`$99` | Power-up: tank (type 6) | `$E31C` (DrawPowerUp type=6: tile=`$99`; OAM `$99` → PT1 `$98`–`$99`) | Extra life power-up icon |
| `$9A`–`$9F` | *Found in data, purpose unclear* | `$D1CD` (stage-select BG row: includes `$9E` and `$9F`); `$D1DF` (stage-select row 16: includes `$9D` and `$9C`) | 6 tiles after last power-up pair; appear in stage-select display data; exact visual/semantic purpose not yet identified |
...
| `$B8`–`$C5` | Score popup tiles — "100"/`$B8`–`$BB` (basic), "200"/`$BC`–`$BF` (fast), "300"/`$C0`–`$C3` (power), "400"/`$C4`–`$C7` (armor); drawn by DrawSpawnSprite (`$DFB1`) during kill-end states; formula tile=(`$A8,X`>>3&`$FC`)−`$10`+`$B9` | `$DFB1` (DrawSpawnSprite: tile=(`$A8,X`>>3&`$FC`)−`$10`+`$B9`); OAM bytes: `$B9`(basic), `$BD`(fast), `$C1`(power), `$C5`(armor), `$AD`(P1), `$B1`(P2) | Score popup displayed at kill position; `$A8,X`=0 → skipped. Drawn for states `$10`–`$2F` |
| `$C6`–`$C7` | *Not found in code yet* | — | 2 tiles after spawn-star armor (`$C5`), before eagle BG tiles (`$C8`) |
...
| `$FA`–`$FB` | *Not found in code yet* | — | 2 tiles after kill-anim frame 3, before stage-flag icon tiles |
...
| `$FE`–`$FF` | *Not found in code yet* | — | 2 tiles at end of PT1 bank; `$FF` is also used as the string terminator in all tile data arrays (dual-use: terminator byte AND CHR tile index `$FF`) |
...
- [x] **Implement armor-piercing destruction of steel**: ROM `BulletTileCollision ($E838)` at `$E83E`: if bullet is from player AND `$0101,X` (starLevel) `>= $60` (max star), steel tile is destroyed — `TileDestroyIfNoEntity ($D77F)` clears it to empty. **Fixed**: in `bulletHitsTile()`, steel branch now checks `b.armor`; if true, sets `grid[row][col] = T.EMPTY` before returning true (bullet still stops).
- [x] **Fix stage cycling after stage 35**: After `stageIdx=34` (stage 35), `initLevel(stageIdx+1)` calls `initLevel(35)` which clamps to 34 (stays on last stage). ROM loops — after stage 35 it returns to stage 1 (stageIdx=0). **Fixed**: changed `Math.min(idx, LEVEL_MAPS.length - 1)` to `idx % LEVEL_MAPS.length` in `initLevel()`.
...
- [x] Meta-audit: verify web port completeness and ROM fidelity — **DONE**. Full cross-check of all web/game.js subsystems against ROM disassembly. Findings below.
  ...
  **MISSING / UNIMPLEMENTED:**
  ...
  5. ~~**Eagle size wrong**~~: **Fixed (sessions 10+44).** Normal gameplay eagle is correctly 16×16 BG metatile ($C8–$CB/$CC–$CF via nametable). Explosion animation phases 4/5 now draw 32×32 sprite ($D0–$DF/$E0–$EF) via EagleDrawFull ($E3F2).
  6. ~~**Eagle CHR bank wrong**~~: **Fixed (session 10).** Eagle tiles use PT1/BG bank directly (no +256).
  ...
  8. ~~**Stage-clear tally screen**~~: **Mostly fixed (session 49).** Full-screen layout with ROM nametable positions, per-type score/PTS/count/arrow/sprite rows, timing matches ROM ($CAF1). Remaining: 2P bonus display ($CC48), score accumulation during tally (ROM adds via $DA31 during drain, web pre-counts at kill time).
  ...
  18. ~~**HUD top score strip**~~: **Fixed (session 40).** Moved scores from sidebar to top border row 1; layout follows ROM nametable row 3 column positions (P1 col 2, HI col 11, P2 col 21). Also fixed HUD_BG to `#000000` (was `#282828`), and fixed "IP"→"1P" / "IIP"→"2P" (was using letter 'I' tile $49 instead of digit tiles $31/$32).
...
- [x] **Audit bullet explosion position vs ROM `BulletExplode ($E1AF)`**: **FIXED** — `$E1AF` loads bullet_x→X, bullet_y→Y, sets tile=$B1, then calls `$DAF3`: `SBC #$05` → OAM_X = bullet_x−5. Then `DrawEntityTile ($DABA)`: `SBC #$08` on Y → OAM_Y = bullet_y−8. Tile = $B1+dir×2 (8×16 OAM top-half = & $FE → $B0/$B2/$B4/$B6). **Bug**: game.js had `b.ey = b.y` (missing −8). **Fix**: changed to `b.ey = b.y - 8`. OAM_X = bullet_x−5 was already correct (`b.ex = b.x - 5`). Tile formula `(0xB1 + dir*2) & 0xFE` correct.
...
- [x] **Audit power-up flash tiles vs ROM**: **FIXED (missing flash animation)** — ROM at `$E2EF` draws power-up: if `$62==0` shows blinky icon; if `$62>0` draws flash sprite (OAM tile `$3B`, 8×16 odd-byte → PT1 tiles `$3A`/`$3B` left col, `$3C`/`$3D` right col) at collect position, then decrements `$62`; at `$62=0` clears `$86` (power-up disappears). `$62` is set to `$32` (50) by `PowerUpCollision ($EB4D)` on collect. Tiles `$3A–$3D` in PT1 = PNG indices 314–317 (confirmed visible sprite content). PPUCTRL `$B0` (8×16 sprite mode, BG from $1000). **Bug**: game.js set `powerUp=null` immediately on collection — no flash animation. **Fix**: added `puFlashTimer`/`puFlashPos` state; on collect, save position and set `puFlashTimer=50`; `tickTimers` decrements it; `drawPowerUp` draws `drawSprite16([0x3A, 0x3C, 0x3B, 0x3D], 6, x-8, y-8)` while timer > 0.
...
- [x] **Visual side-by-side comparison**: Systematic code audit performed (session 20) against all documented ROM findings. **Confirmed correct** (CHR path): tank sprites (all 4 dirs, animBit, palette) ✓; eagle intact/damaged/exploding (tiles, palette SP3) ✓; spawn animation (tile seq $AD→$A1→$AD, palette SP3) ✓; power-up icons (CHR sprites, palette SP2) ✓; bullet explosion (tile formula, palette SP2) ✓; HUD kill icons ($6A, position/spacing) ✓; HUD animated tank (tiles $78-$7F, palette SP3, position) ✓; BG terrain all 13 types (TILE_CHR/TILE_PAL) ✓; partial brick rendering (types 0-3 half-wall bitmasks) ✓; coordinate system ✓. **Remaining visual differences** documented as new tasks below (shield CHR tiles, trees z-ordering, HUD tank wiggle).

- [x] **Audit game.js NES_PAL against current ROM-derived values**: Verified. All 8 slots in game.js:25–33 match ROM-derived values exactly (session 20 re-confirmation): BG0–BG3 and SP0–SP3 all identical to the `ROM-derived NES_PAL` block above. No fixes needed.

- [x] **Fix shield rendering to use CHR tiles**: ROM `PlayerShieldDraw ($E330)` draws shield via `DrawTank ($DB0A)` with `$04=$02` (SP2 palette=palIdx 6) and alternating tiles every 2 frames: when `frameCount & 2 = 0` → tile=$29 (DrawTank: left-col $29, right-col $2B); when `frameCount & 2 = 2` → tile=$2D (left-col $2D, right-col $2F). In 8×16 mode: even-frame draws `[TL=$28,TR=$2A,BL=$29,BR=$2B]`, odd-frame draws `[TL=$2C,TR=$2E,BL=$2D,BR=$2F]` — all BG bank (pt1=true). **Fixed (session 21)**: `drawEntity()` now draws CHR shield tiles after the tank body when `chrOff` available: phase=`(frameCount>>1)&1` selects even (`$28,$2A,$29,$2B`) or odd (`$2C,$2E,$2D,$2F`) BG-bank tile set at palIdx 6, transparent=true, at e.x−8/e.y−8. Fallback `fillRect` retained for no-CHR case.
...
- [x] **Write `render_level.py`**: Decodes all 35 stage maps from ROM `$F27D` and renders each as PNG using actual CHR tile graphics + ROM NES palettes. Output: `output_gfx/levels/stage_NN.png` (individual) and `output_gfx/levels/all_stages.png` (7×5 sheet). Uses `TILE_CHR`/`TILE_PAL` tables matching game.js and correct BG0–BG3 palettes. **Done.**

- [x] **Fix `render_sprites.py` palette and tile-order bugs**: Several palette and tile-order issues were carried forward from early RE sessions. Fixes applied: (1) Spawn animation: NES_PAL[4]→NES_PAL[7] (SP3, per ROM `DrawShootSprite $E0BF`); (2) Bullet explosion: NES_PAL[4]→NES_PAL[6] (SP2, per ROM `BulletExplode $E1AF`); (3) Eagle: NES_PAL[6]→NES_PAL[7] (SP3, per ROM `EagleStateUpdate $E386`); (4) Eagle tile order: fixed to row-major `[D1,D3,D5,D7, D9,DB,DD,DF]` per REVERSE.md; (5) Power-up palette: NES_PAL[7]→NES_PAL[6] (SP2, per session 13); (6) Power-up tile order: `T,T+1,T+2,T+3` → `T,T+2,T+1,T+3` (per session 13). **Done.**

- [x] **Document NMI draw loop and replicate as `render_frame.py`**: NMI handler at `$D300` fully traced — see NMI Handler section in REVERSE.md. Memory map corrected: `$0400–$07BF` = nametable tile shadow (960 bytes, 32×30 tile indices, mirrors PPU $2000–$27BF via PPUQueueTiles $D6D3 side-effect writes); `$07C0–$07FF` = attribute shadow (64 bytes). `render_frame.py` written: takes `--ram <2KB_dump>` (reads OAM at $0200–$02FF and nametable shadow at $0400–$07FF) or `--stage N` (synthetic frame from ROM level data); renders full 256×240 NES frame (behind-BG sprites → BG tiles → front sprites) using CHR PT1/PT0 banks and ROM PaletteData ($D44A); outputs PNG. CHR mapping: `chr_data[0x0000–0x0FFF]` = PT1 BG tiles 0–255; `chr_data[0x1000–0x1FFF]` = PT0 sprite tiles 0–255. Attribute table lookup: `attr_pal(col,row)` = `(attr[$07C0 + (row>>2)*8 + (col>>2)] >> (((row>>1)&1)*4 + ((col>>1)&1)*2)) & 3`. **Done.**
...
### Stage tile rendering fixes

#### Findings from `dump_tiles.py --all-terrain`

- **Tile 0x00 is NOT blank**: 23 non-zero pixels; pattern concentrated at right edge and bottom rows — looks like the complement/shadow of tile 0x0F (brick). Pixels use color indices 1–3, so with BG0 palette they render as orange/red/white, not black. The `if tile_idx == 0x00: continue` skip in `render_level.py` and `render_frame.py` is wrong — it leaves black where the ROM would actually draw tile 0x00.
- **Tile 0x0F (brick half)**: 38 non-zero pixels; upper rows dense (brick body), lower rows sparser (mortar rows) — correct brick half-tile.
- **Tile 0x10 (steel filled half)**: 44 non-zero pixels; dense bottom-right region — the "filled" steel quadrant.
- **Tile 0x11 (steel open alt)**: 23 non-zero pixels; dense top-left region — visually the vertical flip of 0x10.
- **Tile 0x12 (water)**: 38 non-zero pixels; wavy 2-row wave pattern — looks correct.
- **Tile 0x20 (steel open half)**: 30 non-zero pixels; sparse, right-edge heavy — the "open" steel quadrant, complement of 0x10.
- **Tile 0x21 (ice)**: 52 non-zero pixels; dense geometric pattern — correct ice tile.
- **Tile 0x22 (trees)**: 22 non-zero pixels; sparse leafy pattern in lower half — correct trees tile.
...
- [x] **Add border and eagle to render_level.py stage previews**: **Done (session 22)**. Added `BORDER_PX=32` (2 NES tiles), `EAGLE_AREA=64` (2 metatile rows). Canvas changed from 416×416 to 480×544. Grid drawing offset by `BORDER_PX` (32px) in both axes. Added `draw_eagle()`: draws 4×4 BG-bank CHR tile grid (EAGLE_INTACT_TILES rows `[$D0,$D2,$D4,$D6]`/`[$D1,$D3,$D5,$D7]`/`[$D8,$DA,$DC,$DE]`/`[$D9,$DB,$DD,$DF]`) at canvas (192, 448) = metatile (col=5, row=13), with `EAGLE_PAL` = SP3 `[$0F,$04,$16,$20]`. Sheet size updated to 3392×2744 (7 cols × 484 + 5 rows × 548). Output verified: stage_01.png shows black border on all sides + intact eagle at bottom centre in SP3 palette.

- [x] **Audit game.js partial-brick TILE_CHR**: Verified TILE_CHR types 0–3 are `null` (handled via brickBits). The draw loop was skipping empty quadrants entirely. Tile `$00` is NOT blank (23 non-zero pixels = shadow/mortar pattern). **Fixed (game.js:1015–1023)**: changed brick draw loop to always draw all 4 quadrants — set bits draw tile `$0F` (solid brick), unset bits draw tile `$00` (shadow/mortar), both with `TILE_PAL[T.BRICK]` (BG0 palette). Consistent with render_level.py fix (session 1).

### Web port rendering — remaining issues (session 23)

Level tiles and eagle still render incorrectly in the browser. Root causes identified below.

**Critical — `putImageData` transparency compositing bug:**

- [x] **Fix `drawCHRTile` to use `drawImage` instead of `putImageData` for transparent tiles**: **Done (session 6).** Changed `drawCHRTile()` (game.js:57–92) tile cache from `Map<string, ImageData>` to `Map<string, Canvas>`. Each cached tile is now a `document.createElement('canvas')` sized `8*SCALE × 8*SCALE`. Pixel data is built into an ImageData, `putImageData`'d onto the offscreen canvas once during cache creation, then blitted to the main canvas via `ctx.drawImage(cached, ...)` which properly composites alpha. Transparent tiles (color-0 pixels left at RGBA 0,0,0,0) now correctly show the background underneath instead of erasing it. Fixes all transparent sprite rendering: eagle, tanks, spawn animation, shield, power-ups, trees overlay, explosions.

**Eagle rendering:**

- [x] **Audit and fix eagle wall positions in `drawEagleBase()`**: **Done (session 7).** Disassembled `$C912` (BrickWallInit) and `$C9BB` (SteelWallFortify). Both write to nametable rows 24–27 starting at col 12 via `$D6D3` (PPU nametable write helper, uses `$D5FC` to convert tile X,Y → PPU address). Data at `$D22D`–`$D248` (brick) / `$D249`–`$D264` (steel): Row 24 all empty; Row 25 = [00,**0F**,**0F**,**0F**,**0F**,00]; Row 26 = [00,**0F**,C8,CA,**0F**,00]; Row 27 = [00,**0F**,C9,CB,**0F**,00]. Wall tiles ($0F brick / $10 steel) form a Π (inverted-U) of 8 individual 8×8 tiles: top bar (4 tiles at ey-16, x from ex-16 to ex+8) + left leg (2 tiles at ex-16, ey-8 and ey) + right leg (2 tiles at ex+8, ey-8 and ey). Eagle BG tiles (C8/CA/C9/CB) are the 16×16 center at (ex-8,ey-8). Level grid has type 13 (empty) at all eagle-area positions (metatiles 5–7, rows 11–12), so `drawEagleBase()` must draw the walls. Fixed game.js: replaced 4 `drawMetatile()` calls (which drew 32×32 covering the eagle) with 8 `drawCHRTile()` calls in the correct Π positions. Fallback (no chrOff) also fixed to draw Π-shaped rects instead of a solid 32×32 block.
...
- [x] **Replace all `fillText`/canvas font usage with CHR tile text rendering**: **Done (session 9).** Added three new functions to game.js: `drawBigCHRTile(tileAbs, palIdx, destX, destY)` — reuses normal 8×SCALE cached tile and draws it at 32×SCALE via `ctx.drawImage` with `imageSmoothingEnabled=false` for pixel-perfect 4× magnification; `drawNesText(str, x, y, palIdx)` — maps each ASCII char (0x21–0x5F) to its CHR BG-bank tile index, calls `drawCHRTile` per character, with canvas font fallback when `!chrOff`; `drawBigNesText(str, x, y, palIdx)` — same but using `drawBigCHRTile` for 32×32 magnified characters, with fallback. Replaced all 14 `fillText`/`text()` call sites: HUD enemy icons (removed "ENEMY" label, icons start at hy+2), stage number (flag icon $6B + digit tiles), player "IP" label + tank icon $14 + lives digit, score (6-digit zero-padded), freeze indicator, stage banner ("STAGE  N" centered), game over (big text "GAME"/"OVER" at 64,84/64,116 + "SPACE TO RETRY"), stage clear tally (title, type names, kill counts "NX100", row scores, TOTAL line), and controls hint ("ARROWS:MOVE  SPACE:FIRE"). All primary text now uses BG3 palette (palIdx 3, white/gray). Canvas font `fillText` only remains in: `text()` helper (HUD no-CHR fallback), eagle dead "✕" fallback, entity type number fallback — all inside `!chrOff` branches.
...
- [x] **Generate reference comparison**: **Done (session 11).** Ran `render_frame.py --stage 1` and wrote `analyze_tiles.py` (no external dependencies — built-in PNG reader/writer) to render both ROM-accurate and game.js-equivalent BG layers at 1× NES resolution (256×240), then pixel-diff them. Output: `output_gfx/compare_rom.png`, `compare_gamejs.png`, `compare_diff.png`. Results:
  ...
  **Sub-tasks to fix:**
- [x] **Fix C.FIELD background color**: Changed `C.FIELD` from `'#080808'` to `'#000000'` in game.js (line 255). NES universal background = palette[0][0] = NES $0F = pure black. Fixes 24,576 playfield_bg pixel diffs.
- [x] **Remove non-NES border decoration**: **Done (session 8).** Removed the gray `#404040` border `fillRect` from `drawField()` (was line 1146) and deleted the unused `C.BORDER` constant (was line 256). The NES has no such border — border area will be handled by the next task (render nametable border tiles). Playfield fill with `C.FIELD` (`#000000`) remains. Fixes 5,120 border_area pixel diffs.
- [x] **Render nametable border tiles**: **Done (session 9).** Added `drawBorderTiles()` to game.js (before `drawField`). Renders CHR tile $00 with BG0 palette (palIdx 0) at all 284 nametable positions outside the 26×26 playfield tile grid: top 2 rows (rows 0–1, 32 cols), bottom 2 rows (rows 28–29, 32 cols), left 2 cols (rows 2–27), right 4 cols (rows 2–27, cols 28–31). Called from `drawField()` after the black fill and before tile/eagle rendering. HUD (`drawHUD()`) renders on top afterward. Fixes 4,692 outside_playfield pixel diffs.

### CHR bank investigation — tiles/sprites look garbled (session 24)

ROM has 4 CHR banks (16KB total). Mapper 99 (VS System) can swap between bank pair 0+1 and pair 2+3 via $4016 bit 2. **Banks 0-1 differ from banks 2-3 by 3820+ bytes each** — completely different tile graphics. Current `extract_tiles.py` / `game.js` / all render scripts use banks 0+1 only. **Confirmed: gameplay uses banks 2+3 for most stages (including stage 1).** ALL CHR tile rendering from banks 0+1 is wrong.

Evidence: tile $00 in bank 0 = mortar pattern (23 non-zero px), tile $00 in bank 2 = "O" character shape. Completely different content.

**CHR bank select mechanism (session 25):**
- `StageFirstTimeInit` ($871C): `LDA $59; AND #$FB; ORA StageFlagsTable,X; STA $59` — sets bit 2 of `$59` ($4016 shadow) per stage
- `StrobeControllers` ($9A23): writes `$59` to `$4016` during main-loop controller reads → bit 2 selects CHR bank pair
- NMI `$D68A`: writes `$01/$00` to `$4016` (clears bit 2) during vblank only — does not affect visible rendering
- `StageFlagsTable` ($80B6, 14 bytes): `04 04 04 00 04 04 04 04 04 00 00 04 00 00` — `$04` = banks 2+3, `$00` = banks 0+1
- **Banks 2+3 = CHR file offsets $A010 (BG patterns, 4KB) + $B010 (sprite patterns, 4KB)**

- [x] **Determine which CHR bank pair is active during gameplay**: **Done (session 25).** CHR banks 2+3 are active during gameplay for most stages (including stage 1). Mechanism: `StageFirstTimeInit` ($871C) loads `StageFlagsTable[$41]` from $80B6 and ORs it into `$59` (the $4016 shadow register) after clearing bit 2. For stage 0 (and most stages), the flag is `$04` (bit 2 set → CHR banks 2+3). `StrobeControllers` ($9A23) writes `$59` to `$4016` during each main-loop controller read, preserving bit 2. The NMI handler's `$D68A` briefly writes `$01/$00` to `$4016` (clearing bit 2) during vblank for its own controller read, but this is invisible — rendering uses the main-loop state. `StageFlagsTable` at $80B6 (14 entries): stages 0,1,2,4,5,6,7,8,11 → `$04` (banks 2+3); stages 3,9,10,12,13 → `$00` (banks 0+1). **Conclusion: the web port and all Python render scripts must use CHR banks 2+3 (file offsets $A010 BG / $B010 sprites) for standard stage 1 gameplay. Current banks 0+1 are wrong.**
- [x] **Extract both CHR bank pairs and visually compare**: **Done (session 26).** Created `analyze_chr_banks.py` which extracts both bank pairs and outputs `output_gfx/chr_bank01.png`, `output_gfx/chr_bank23.png`, and `output_gfx/chr_compare.png` (side-by-side key tiles). Results: **all 512 tiles differ** between bank pairs — 0/256 BG identical, 0/256 sprite identical. Both banks contain complete gameplay tile sets (terrain, tanks, eagle, power-ups, text) but with entirely different art. Bank 0+1: BG $00 = mortar pattern, has ASCII 0-9/A-Z text tiles, 0 blank BG tiles, 8 blank sprite tiles. Bank 2+3: BG $00 = "0" numeral glyph, different terrain/sprite art style, 1 blank BG tile, 12 blank sprite tiles. Hex comparison — BG brick $0F: bank 0 `p0=740cf8feaaaa0000` vs bank 2 `p0=0000008080800080`; BG steel $10: bank 0 `p0=004077547b5a7a59` vs bank 2 `p0=f0d8cc64248cd870`. **Conclusion confirmed: banks are two complete visual themes; StageFlagsTable selects per-stage. Stage 1 uses bank pair 2+3 ($A010/$B010). Both are valid gameplay graphics — the game alternates themes across stages.**
- [x] **Fix `extract_tiles.py` to use correct CHR bank**: **Done (session 27).** Rewrote `extract_tiles.py` with `--bank` flag (0 or 1, default 1) and `--all` flag. Default now extracts bank pair 1 (mapper banks 2+3, file offset $A010-$C00F) — the correct gameplay bank for stage 1. Outputs: `tiles/chr_all.png` (512 tiles, 32x16), `tiles/chr_bg.png` (BG tiles 0-255), `tiles/chr_sprites.png` (sprite tiles 0-255). `--bank 0` selects banks 0+1 ($8010). `--all` generates both pairs plus combined sheet. Old `chr_pt0.png`/`chr_pt1.png` replaced by `chr_bg.png`/`chr_sprites.png`. Verified: bank 1 tile $00 = "0" numeral (`p0=384cc6c6c6643800`), bank 0 tile $00 = mortar pattern (`p0=000001014171557c`), matching RE findings.
...
- [x] **Fix eagle rendering in game.js**: **Done (session 33).** Eagle rendering in game.js was already correct — `drawCHRTile(T & 0xFE, 7, ...)` correctly reads from BG/PT1 bank (tiles 0-255 in chr_all.png), which is correct for odd OAM tiles ($D1-$DF → bit0=1 → PT1). The 696px diff was a bug in **compare_frames.py's reference render**, which incorrectly used `chr_spr` (PT0/sprite bank) for eagle tiles instead of `chr_bg` (PT1/BG bank). Fixed reference to use `chr_bg`. Diff now: 5px (0.008%) — negligible transparent pixel overlap at eagle/wall boundary. No game.js changes needed.

- [x] **Fix HUD text transparency**: **Done (session 34).** Code verified correct: `drawCHRTile` skips color-0 pixels when `transparent=true` (line 76). `drawNesText` passes `transparent=true` for all characters (line 142). HUD icons ($6A enemy, $6B flag, $14 tank) all pass `transparent=true`. `drawBigCHRTile` (GAME OVER text) reuses transparent cache. `compare_frames.py` confirms 0 HUD mismatches. HUD background (`C.HUD_BG`) drawn first at line 1511, transparent tiles overlay correctly.

- [x] **Implement in-field scrolling GAME OVER text**: **Already implemented.** game.js has full in-field scroll: `goScrollTimer` (ROM $0108) starts at 17 on player death (no lives) or eagle destruction; Y scrolls up 1px/frame while timer≥10; timer decrements every 16 frames; draws BG-bank CHR tiles $78-$7F (palette SP3) as two 16×16 sprites ("GAME"+"OVER") at scroll position (lines 1689-1701); transitions to big overlay when timer=0. Variables: `goScrollY`/`goScrollTimer`/`goScrollFrame` (lines 310-312), init at level start (lines 388-390), trigger paths for player death (lines 857-861) and eagle destruction (lines 1052-1056).

- [x] **Add per-stage CHR bank switching**: **Done (session 35).** Mapper 99 StageFlagsTable ($80B6, 14 entries) selects CHR bank pair per stage: D2=1 ($04) → banks 0+1 for stages 0,1,2,4-8,11; D2=0 ($00) → banks 2+3 for stages 3,9,10,12,13. Updated `extract_tiles.py` to output bank-specific filenames: `chr_all.png` (default, banks 0+1) and `chr_all_alt.png` (alt, banks 2+3). Updated `game.js`: added `STAGE_CHR_BANK[14]` array from $80B6; `initCHR()` loads both tile sheets into `chrSheets[2]`; `setCHRBank(stageIdx)` switches `chrOff` to correct bank and clears `tileCache`; called from `initLevel()` at stage start. Stages using alt bank pair now render with correct CHR graphics (different terrain art, text font, tank sprites).

- [x] **Implement title/attract screen**: **Done (session 36).** Added `'title'` gamePhase. ROM `$C65C` AttractWait loops with `BlinkTitleSprite ($C69A)` toggling text at bit 5 of frame counter (~1 Hz). ROM `$CFAA` PreGameDraw draws "BATTLE" at (26,46) and "CITY" at (60,86) via `DrawSpriteString ($D14F/$D156)` as 32×32 magnified characters. Implementation: `enterTitle()` sets gamePhase='title', titleFrame=0; `drawTitleScreen()` renders black screen + `drawBigNesText('BATTLE',26,46,3)` + `drawBigNesText('CITY',60,86,3)` + "NAMCO 1985" + blinking "PRESS START" (toggles every 32 frames via `titleFrame & 0x20`); `update()` title handler waits for Space/Enter then resets score/lives and calls `initLevel(0)`; gameover now returns to title via `enterTitle()` instead of restarting level. Boot sequence changed from `initLevel(0)` to `enterTitle()`.

- [x] **Implement 2-player mode**: **Done (session 37).** P2 entity slot (index 1) fully implemented with simultaneous 2-player: `numPlayers` variable (1/2) set from title screen menu; P2 state vars (`p2Score`/`p2Lives`/`p2NextLifeScore`); P2 input (WASD+E/Q separate from P1 arrows+Space); P2 spawn/respawn timers; score routing via `e.lastHitBy`; bullet-bullet cancel includes P2 slots [0,1,8,9]; 1-Up routes to collecting player; HUD shows P2 lives+score in 2P mode; gameover requires both players out of lives; title screen Up/Down toggles 1P/2P selection with cursor.

- [x] **Implement victory screen**: **Done (session 38).** ROM `$C44D` (DrawVictoryScreen) shows "PEACE BE WITH YOU" + decorative tiles + 240-frame ScrollX slide after completing all 35 stages. Implementation: added `'victory'` gamePhase with 4 phases: phase 0 holds "PEACE BE WITH YOU" (BG3 text at col7 row10 = 56,80) + 9 decorative CHR tiles $60-$68 (at col12 row14 = 96,112) for 180 frames; phase 1 scrolls horizontally (victoryScrollX 0→240 over 240 frames, mimicking ROM `INC $4F` loop); phase 2 holds "NOW LONG WAR" (80,56) + "COMES TO" (96,80) + "AN END" (104,104) for 240 frames; phase 3 blinks "PRESS START" and waits for keypress → `enterTitle()`. Triggered from 'clear' phase when `stageIdx === LEVEL_MAPS.length - 1` (stage 34 = final stage). `drawVictoryScreen()` renders full-screen black + scrolling text with BG3 palette (palIdx 3).

- [x] **Implement hi-score tracking**: **Done (session 39).** ROM `$D9F0` (CompareAndUpdateHiScore) compares P1/P2 scores vs hi-score buffer ($3D–$43), returns Y=0/1/$FF; `NewHiScoreDisplay ($C4E9)` draws "HISCORE" label ($D16B) + digits ($D9C4) with random palette flash. Implementation: added `hiScore` variable (default 20000) and `newHiScorePlayer` (0=none, 1=P1, 2=P2); on gameover transition, compares p1Score/p2Score vs hiScore and updates; `drawGameOver()` shows "HISCORE" + value with cycling palette flash when `newHiScorePlayer > 0`; HUD displays "HI" label + 6-digit hi-score between P1 score and P2 section; title screen shows "HI-" + hi-score below copyright. Hi-score persists across games within a session (not saved to localStorage). Reset to 20000 only on page reload.

- [x] **Add sound engine**: **Done (session 8).** Web Audio API implementation in `web/sound.js`: 4 channels (2×pulse/square, 1×triangle, 1×noise via looped AudioBuffer); 28-slot sequence interpreter matching ROM $EC23 engine (note decoding via $EE8B pitch table, duration $61-$E7, 3-level loops $F0-$F2, volume cmds $E9-$EE, stop $E8); all 28 sound sequences extracted from ROM and embedded as JS arrays. BGM: 3-slot looping (pulse1 $EEDB + triangle $EF02 + pulse2 $EF2D), auto-restart via `tickBGM()`. SFX triggers added to `game.js`: player/enemy fire, brick/steel/water hit, armor hit, entity kill (noise burst + explosion), eagle hit, power-up appear, extra life (P1/P2), stage clear melody (3-part), game over (BGM stop), victory melody (3-part), title return (stopAllSounds). Sound toggle via M key. `initAudio()` called on first user gesture (title screen Start).

### Web port HUD/rendering audit (session 40)

Systematic comparison of `game.js` rendering against ROM nametable/OAM layout documented in REVERSE.md. Items marked with ROM references for implementation.

- [x] **Stage flag icon: use correct 2×2 tiles ($6C/$FC/$6D/$FD)**. **Done (session 41).** Web was drawing single tile $6B at rows 23. ROM uses 4 BG tiles in a 2×2 grid at nametable rows 23–24, cols 29–30 (SrcPtr=$D225/$D228): top-left=$6C, top-right=$FC, bottom-left=$6D, bottom-right=$FD, all BG3 palette. Fixed game.js:1714–1718: replaced single `drawCHRTile(0x6B,...)` with 4 `drawCHRTile` calls at pixel positions (232,184)/(240,184)/(232,192)/(240,192). Stage number digits moved to row 25 (sty+16).
- [x] **HUD bouncing tank animation ($C7F8)**. **Done (session 42).** The "HUD bouncing tank" is actually the in-field "GAME OVER" text scroll (tiles $78-$7F = 32×16px via 4 OAM 8×16 sprites). Web already had partial implementation (goScrollY, direction UP only, hardcoded X=112). Enhanced to match full ROM behavior: added `goScrollX` (ROM $0105) and `goScrollDir` (ROM $0107) state variables; implemented 4-direction wiggle via ROM tables ($D2C6/$D2CA); added `startPlayerDeathScroll()` for 2P per-player death (ROM $DECC): P1 dies→scroll from X=$20 right, P2 dies→scroll from X=$C0 left, both→center up. Draw code now uses variable `goScrollX` instead of hardcoded 112. Gameover transition gated: only fires when all players dead or eagle destroyed (2P per-player scroll just hides).
- [x] **HUD sidebar column alignment**. **Done (session 43).** Changed `hx` from `FX + GW*META + 6` (=230, col 28.75) to `27 * 8` (=216, col 27). Background fill width 36→40px (covers cols 27–31). All sidebar items repositioned to ROM-accurate nametable columns: enemy icons at cols 29–30 (`hx+16`), "1P"/"2P" labels at col 29 rows 17/20 (ROM $C72D/$C746 via $D6D3), player icon tile $14 at col 29 rows 18/21 (ROM $C6C5), lives digit at col 30 (`hx+24`). Verified ROM disassembly: $C72D draws $D161 (tiles $58,$13=1P) at X=$1D Y=$11; $C746 draws $D164 (tiles $5A,$13=2P) at X=$1D Y=$14; $C6C5 draws $D212 (tile $14) at X=$1D Y=$12/$15.
- [x] **Eagle 32×32 sprite during explosion animation**. **Done (session 44).** ROM `EagleStateUpdate ($E386)` phases 4 (intact) and 5 (damaged) call `EagleDrawFull ($E3F2)` which draws 4×`DrawSprite16` = 8 OAM 8×16 entries = 4×4 grid of 8×8 tiles = 32×32px at (ex−16,ey−16), palette SP3 (palIdx 7). Intact tiles $D0–$DF, damaged $E0–$EF, PT1/BG bank. Web was drawing 16×16 BG metatile ($C8–$CB/$CC–$CF) for phases 4/5. Fixed: replaced `drawMetatile` with 4×4 `drawCHRTile` loop; tile formula `tBase + col*2 + (row&1) + (row>>1)*8`. Note: normal gameplay eagle ($68=$80, BMI→RTS) is correctly 16×16 BG metatile via nametable — the 32×32 sprite is **only** during explosion animation ($68=1–39). Commit `46cc2e7` correctly reverted the earlier over-broad 32×32 change.
- [x] **Game-over hi-score flash: use RNG palette instead of sequential cycling**. **Done (session 45).** ROM `NewHiScoreDisplay ($C4E9)` loop at $C524: each frame calls RNG ($D37C), ANDs with $3F (0–63 NES color), writes to PPU $3F01 (BG0 color 1) via QueuePaletteWrite ($DB3E). Web was cycling sequentially `(frameCount >> 2) & 7` (changed every 4 frames). Fixed: replaced with `Math.floor(Math.random() * 8)` — random palette each frame, matching ROM's per-frame RNG behavior.
- [x] **Enemy icon erased slots should draw tile $11**. **Done (session 46).** ROM `DrawHUDKillIconB ($C7AE)` writes tile $11 (blank/steel) when enemy spawns, erasing from bottom-right to top-left. Web was only drawing $6A icons for remaining enemies and leaving erased slots empty. Fixed: loop now always iterates all 20 slots; indices ≥ `total` draw tile $11 via `drawCHRTile(0x11, 3, ...)` when CHR loaded. Non-CHR fallback unchanged (background fill covers empty slots).
- [x] **Score leading-zero suppression (SkipLeadingZeros $D9A7)**. **Done (session 47).** ROM `SkipLeadingZeros ($D9A7)` scans BCD digit buffer, skipping zero bytes until first non-zero — leading zeros become blank tiles (not drawn). Web was using `padStart(6, '0')` which rendered '0' chars. Fixed: changed all 5 score `padStart` calls from `'0'` to `' '` (space). `drawNesText` skips space chars (code < 0x21), producing blank gaps matching ROM. Affected: P1 score, HI score, P2 score (HUD strip), hi-score on game-over screen, hi-score on title screen.
- [x] **Title screen: verify copyright text + mode select positions against ROM**. **Done (session 48).** ROM `PreGameDraw ($CFAA)` title screen layout verified: score strip at nametable row 3 (y=24) with HI label at col 11; "BATTLE" DrawSpriteString at (26,46), "CITY" at (60,86) (each char +$20=32px advance); decoration tiles $60-$68 at (col 13, row 23); copyright `$D1E7` = "@ 1980 1985 NAMCO LTD" + tile$69 at (col 5, row 25) → pixel (40,200); `$D1FE` = "ALL RIGHTS RESERVED" at (col 7, row 27) → pixel (56,216); `BlinkTitleSprite ($C69A)` alternates $D1A7 "PLEASE INSERT COIN" / $D1BA (18 spaces) at (col 7, row 18). **VS System has NO mode select** — coins determine play mode. Web fixes: (1) copyright text changed from "NAMCO 1985" at (80,126) to "@ 1980 1985 NAMCO LTD" at (40,200) — '@' = CHR tile $40 = © symbol; (2) added "ALL RIGHTS RESERVED" at (56,216); (3) HI score moved from (80,134) to (88,24) matching ROM row 3; (4) mode select repositioned from (88,142)/(88,158) to (88,136)/(88,152) — web-only feature at rows 17/19; (5) "PRESS START" moved from (76,178) to (72,168); (6) controls hint moved from (20,210) to (16,228)
- [x] **Stage-clear tally screen: verify positions + animation timing against ROM**. **Done (session 49).** ROM `StageClearTallyScreen ($CAF1)` → `TallyScreenInit ($CD04)` full nametable layout verified. Full-screen black (not overlay panel). Layout: row 3 "HI-SCORE" (col 8) + value (col 18); row 5 "STAGE" (col 12) + number (col 14); row 7 P1 label "$5E-PLAYER" (col 3); row 9 P1 score (col 5); per-type rows at nametable rows 12/15/18/21 (y=96/120/144/168): per-type score (col 1→x=8), "PTS" ($D21E, col 8→x=64), kill count (col ~13→x=96–104), arrow tile $5B (col 14→x=112), enemy 16×16 sprite at (121, ry-4) via `TallyDrawSprite16 ($CF3C)` with tile bases $80/$A0/$C0/$E0; row 22: 7× separator tile $5C (col 12→x=96); row 23: "TOTAL" ($D275, col 6→x=48) + total kill count (col ~13). Timing: initial pause 25 frames ($CAF4 LDX #$19), per-tick 7 frames ($CBF5 LDX #$07), inter-row 18 frames ($CC09 LDX #$12), final hold 100 frames ($CCF3 LDX #$64). Bonus section ($CC48): 2P only, compares P1/P2 totals, awards bonus + "BONUS" string ($D283) at row 25. Web fixes: (1) changed from 192×100 overlay panel to full-screen render with early return in render(); (2) replaced TYPE_NAMES labels with ROM-matching "score PTS count ← [sprite]" layout; (3) positions match ROM nametable col×8/row×8; (4) timing: initial 60→25, per-tick 4→7, inter-row 12→18, final hold 180→100 frames
- [x] **Power-up sprites: bottom half duplicated as top half**. **Fixed (session 40).** `drawSprite16` with `pt1=true` applied `t & 0xFE` to all tile indices, mapping odd indices (bottom halves: $81→$80, $83→$82) to even (top halves). Affected both power-up sprite (`[base, base+2, base+1, base+3]`) and power-up flash (`[$3A,$3C,$3B,$3D]`). Fix: changed `pt1` path from `t => t & 0xFE` to `t => t` since callers already pass correct 8×8 tile indices.
- [x] **Alt CHR bank border tile $00 is decorative, not blank**. **Done (session 12/50).** ROM `ClearNametableSlot ($98EB)` fills nametable with tile `$FC` (not `$00`). Bank 1 (default stages) tile $FC = decorative gray→brown diagonal ramp (64 non-zero px, BG0 palette colors 1+3). Bank 3 (alt stages) tile $FC = blank (all zeros → invisible black border). Playfield empty cells use tile $00 (all zeros in both banks). Web fixes: (1) `drawBorderTiles()` changed from tile $00 to $FC; (2) `render_frame.py` nametable init changed from `bytearray(NT_TILES)` to `bytearray([0xFC] * NT_TILES)`; (3) `compare_frames.py` same; (4) added type 13 (EMPTY) → `[$00,$00,$00,$00]` to TILE_CHR in both scripts so empty cells override the $FC fill. Label added: `ClearNametableSlot` at $98EB.
- [x] **Eagle wall bricks are visual-only, not in collision grid**. **Done (session 13/51).** Added `EAGLE_WALL` constant with 5 metatile positions (row 11 cols 5-7 + row 12 cols 5,7) and partial `brickBits` masks (0b1000/0b1100/0b0100/0b1010/0b0101). `setEagleWall(steel)` sets grid cells to T.STEEL or T.BRICK with correct brickBits. Called from: (1) `initLevel()` → `setEagleWall(false)` after grid copy; (2) shovel power-up → `setEagleWall(true)` for steel fortification; (3) shovel timer expiry → `setEagleWall(false)` to revert; (4) shovel flash (last 4 ticks) → alternates via `(frameCount >> 4) & 1`. Removed manual wall drawing from `drawEagleBase()` — walls now rendered by `drawTile()` loop. Eagle zone collision box narrowed from 24×16 to 16×16 (walls handle their own blocking via grid). (ROM $C912/$C9BB)

### Open tasks (session 52)

- [x] **Verify border tile rendering in running web game against emulator**. **Done (session 52).** Root cause confirmed: `drawHUD()` filled cols 27–31 with flat black `C.HUD_BG`, overwriting the tile $FC background that `drawBorderTiles()` had drawn. ROM investigation: `ClearNametableSlot ($98EB)` fills entire nametable with $FC and all 64 attribute bytes with $00 (palette 0/BG0). Level loader ($D82B→$D613) writes attributes only for playfield metatiles (tile cols 2–27, attribute cols 0–6). HUD area (tile cols 28–31, attribute col 7) attributes are NEVER modified — stays $00 = palette 0 (BG0). HUD tile writes via `PPUQueueTiles ($D6D3)` write tile data only, no attribute changes. **Web fix**: (1) Replaced `fillRect(hx, hy, 40, ...)` with tile $FC draws (palette 0) at cols 28–31 rows 2–27 in CHR mode; fallback mode uses narrower black fill at col 28 (width 32) instead of col 27 (width 40). (2) This also fixes a bug where col 27 (rightmost playfield column) was being erased by the HUD background. **Note**: Web currently uses palette 3 (BG3) for HUD icons/text, but ROM attribute table shows palette 0 (BG0) — tracked as follow-up.

- [x] **HUD elements use wrong palette**: **Done (session 53).** Changed all `drawCHRTile(..., 3, ...)` and `drawNesText(..., 3)` calls in `drawHUD()` to use palette 0 (BG0). Affected elements: enemy icons ($6A), blank slots ($11), lives icon ($14), "1P"/"2P" labels, lives digits, flag icon ($6C/$FC/$6D/$FD), stage number, "FREEZE" text, and top score strip (1P/HI/2P scores). HUD sidebar now renders with BG0 palette [0x0F,0x17,0x06,0x00] = orange-brown tones matching ROM attribute table ($00 = palette 0 for attr col 7).

- [x] **Investigate "half metasprite" (8px) positioning offset**. **Done (session 55).** Investigation found no systematic 8px offset in entity positions (center coords, spawn values, OAM draw offsets all match ROM). Found and fixed two positioning bugs that could cause perceived misalignment: (1) **Bullet spawn `*9` → `*8`** (game.js:784–785): ROM `FireBullet ($E140)` at $E156 uses `LDA $E529,Y; ASL; ASL; ASL` (direction delta × 8) + entity center → bullet position. Web used `DX[e.dir] * 9` / `DY[e.dir] * 9` — 1px too far from tank barrel. Fixed to `* 8`. (2) **Grid snap on 180° turns** (game.js:683–688): ROM `PlayerInputUpdate ($DC23)` at $DC77–$DC93 only applies `(+4)&$F8` grid snap for **perpendicular** turns; same-direction and 180° reversals (`EOR #$02` at $DC7D) skip the snap entirely. Web was snapping on ALL direction changes including 180°, causing a visible jerk/shift when reversing. Fixed: added `d !== (e.dir ^ 2)` guard so 180° reversals skip grid snap.

### Open tasks (session 56)

- [x] **Implement Stage Selection screen**. **Done (session 56).** Added `selectedStage` (ROM $85) and `'select'` phase. Implement `drawStageSelect()` matching ROM $C8B6: gray background with $FC border tiles + "STAGE XX" banner. Transitions from title screen to select phase; Up/Down cycles 35 stages; Space/Enter starts selected level.
- [x] **Implement Attract Mode (Demo)**. **Done (session 56).** Added `titleTimer` (ROM $0A counts to 8 before demo) and `demoMode` (ROM $6D). If title is idle for 600 frames (10s), starts Stage 35 (index 34) demo. Implemented `tickDemoAI()` providing automated player movement/firing. Input functions (`p1Dir`, `p2Dir`, `handlePlayerFire`) use AI inputs in demo mode. Eagle/Gameover in demo returns directly to title.
- [x] **Implement Palette Variants (DIP switches)**. **Done (session 56).** Integrated 256-byte `PALETTE_COLOR_TABLE` ($D475) and `NES_MASTER_HEX`. Added `dipSwitch` (ROM $4E) and `updateActivePalette()` ($D46A). Added 'P' key to cycle 4 variants (standard + 3 alternates used in different cabinet types). Tile cache cleared on palette swap.
- [x] **Implement Authentic AI movement logic**. **Done (session 56).** Updated enemy and demo-player AI to use the **exact same** logic as the original ROM: (1) **Axis-Preference ($E543)**: Integrated the 18-byte `DIR_TARGET_TABLE` to resolve diagonal goals with authentic horizontal/vertical bias sets. (2) **Difficulty Tiers ($DF26)**: AI behavior now evolves from HQ-targeting to Player-hunting based on level duration (`FrameHi` $0A). (3) **Grid-Locked Decisions ($DD30)**: Strategic changes are now gated by 8px tile grid alignment and a 1/16 RNG check, matching the original's organic-yet-structured movement feel. (4) **Stochastic Turns ($DDFC)**: Implemented exact 25% L / 25% R / 50% Goal-based turn distribution.
- [x] **Implement Construction Mode**. **Done (session 56).** Added `edit` phase and `customMap` storage. Press 'E' on title screen to enter editor. Moving cursor with arrows/WASD, cycle tiles with 'T', place with Space/F, save with Enter/Escape. Custom map is loaded when Stage 1 (index 0) is selected.
- [x] **Improve Sound Engine Fidelity**. **Done (session 56).** Refined `sound.js` to match ROM sequence engine ($EE63) precisely: (1) **LFSR Noise**: Replaced white noise with authentic 15-bit Linear Feedback Shift Register generation. (2) **Duty Cycles**: Implemented 12.5%, 25%, 50%, and 25%-negated pulse waveforms using Fourier coefficients. (3) **Volume Commands**: Fixed bit-masking for $E9-$EB volume modification commands.
