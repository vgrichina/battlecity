...
- [ ] **Audit game.js partial-brick TILE_CHR**: Verify game.js TILE_CHR for types 0–3 matches the ROM $DB79 dump results from the task above. If game.js still uses 0x00 for empty quadrants and skips drawing them (transparent canvas), fix to draw tile 0x00 using the brick palette (same fix as render_level.py).
