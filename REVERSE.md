...
- [ ] **Fix `extract_tiles.py` to use correct CHR bank**: Update CHR bank offset based on findings. If banks 2+3 are the gameplay banks, change `chr_off` to point to file $A010 (BG) and $B010 (sprites). Regenerate `chr_all.png` / `chr_pt0.png`. Consider adding `--bank` flag to select bank pair.
