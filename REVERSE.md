...
- [ ] **HUD elements use wrong palette**: Web port draws all HUD elements (enemy icons $6A, "1P"/"2P" text, stage flag, scores) with palette 3 (BG3: black/black/grey/white). ROM attribute table for HUD area (cols 28–31) is all $00 = palette 0 (BG0: black/orange-brown/dark-red/grey). This was confirmed by tracing `ClearNametableSlot ($98EB)` → attributes $00, and verifying no subsequent write modifies attribute col 7 bytes ($07C7/$07CF/$07D7/$07DF/$07E7/$07EF/$07F7). Fix: change all `drawCHRTile(..., 3, ...)` and `drawNesText(..., 3)` calls in `drawHUD()` to use palette 0.

- [ ] **Investigate "half metasprite" (8px) positioning offset**. User reports entities/sprites appear shifted by ~8px in some situations. Initial analysis found no systematic 8px error: entity center coords match ROM (`DrawTank2x2 $DB02`: OAM_X = entityX-8, OAM_Y = entityY-8; web draws at `e.x-8, e.y-8`); player spawn (0x58,0xD8)=(88,216) matches ROM $E537; enemy spawn (0x18/0x78/0xD8, 0x18) matches ROM $E531. Known 1px Y offset from NES OAM Y+1 quirk (not 8px). Need to: (1) run emulator side-by-side with web port and screenshot specific frames; (2) check if the offset appears during movement (grid snap at $DD30 uses `AND #$F0 | #$08` = center-of-metatile snap — verify web port matches); (3) check bullet spawn position relative to tank barrel (ROM $E126 calculates bullet X/Y from entity pos + direction offset); (4) check tally screen sprite positions ($CF3C TallyDrawSprite16 at x=121, ry-4); (5) check power-up and explosion sprite positions.
