...
- [ ] **Implement entity kill explosion animation**: ROM `EntityKillDispatch ($E073)` plays a 3-frame 16×16 explosion sprite at the killed entity's center before clearing the slot. The explosion uses OAM tiles `$84–$8F` (PT0 sprite bank, 3 phases × 4 tiles): `phase0=$84-$87`, `phase1=$88-$8B`, `phase2=$8C-$8F`, each lasting ~4 frames, all palette SP2 (palIdx 6). Currently `killEntity()` sets `e.alive=false` immediately — entity just pops out of existence. Fix: add `e.deathTimer` (12 frames), draw explosion sprite during countdown in `drawEntity()`, clear slot only when timer reaches 0.
- [ ] **Implement stage-clear score tally screen**: ROM `StageClearTallyScreen ($CAF1)` → `TallyScreenInit ($CD04)` shows 4 rows (one per enemy type), drains per-type kill count one-at-a-time with score animation, then totals. Currently game.js shows a placeholder "STAGE CLEAR!" banner. Fix: (1) add `killCounts[4]` array, increment `killCounts[e.type]` in `killEntity()`; (2) replace `drawStageClear()` with animated tally: show type icon + kill count + score per row, drain animation at ~4 frames/kill, lives grant check on score thresholds, then hold 3 s before `initLevel(stageIdx+1)`.
- [ ] **Add lives grant check on score thresholds**: ROM `LivesGrantCheck ($CF44)` at `$EB60` (called from PowerUpCollision and ScoreAdd paths) — award 1 extra life when score passes multiples of 20000. Currently only the 1-Up power-up grants lives. Add threshold check in score-update path.
- [ ] **Implement ICE tile momentum/sliding**: ROM `MoveGridSnap ($DD30)` on ice tile (`$0400` cache tile $1B or $1C): entity continues in current direction even without input, and cannot change direction until it leaves ice. Currently `passable()` marks ice as passable but `moveEntities()` has no sliding logic. Fix: detect ice under entity center (`grid[row][col] === T.ICE`); when on ice, skip direction-change and keep moving in `e.dir` even when no key held.
- [ ] **Implement armor-piercing destruction of steel**: ROM `BulletTileCollision ($E838)` at `$E83E`: if bullet is from player AND `$0101,X` (starLevel) `>= $60` (max star), steel tile is destroyed — `TileDestroyIfNoEntity ($D77F)` clears it to empty. Currently `bulletHitsTile()` always returns `true` for steel with no destruction. Fix: check `b.armor` flag (already set in `tryFire()` when `starLevel >= 0x60`); if true, call `destroyBrick`-equivalent to set `grid[row][col] = T.EMPTY` and return true (bullet still stops).
- [ ] **Fix stage cycling after stage 35**: After `stageIdx=34` (stage 35), `initLevel(stageIdx+1)` calls `initLevel(35)` which clamps to 34 (stays on last stage). ROM loops — after stage 35 it returns to stage 1 (stageIdx=0). Fix: `initLevel((idx) % LEVEL_MAPS.length)`.
- [ ] **Calibrate enemy speed tiers against ROM SpeedTable**: ROM `SpeedTable ($E6A9)` has 36 entries × 4 bytes: `[moveSlice, strafeSlice, fireInterval, aiInterval]`. Current game.js uses rough approximations (type-1 always-processes, others alternate, fire=30+rand*90). Disassemble the 4 entries for type 0–3 at stage 1 to get exact per-frame movement rates and fire intervals; update `moveEntities()` and `handleEnemyFire()` to match.
- [ ] **Fix stale comment on game.js line 1467**: Comment says `chr_pt0.png` but `initCHR()` now loads `chr_all.png`. Update to reflect the correct filename.
- [ ] **Investigate remaining animation discrepancies**: User reports animations still wrong after chr_all.png fix. Run `render_frame.py --stage 0` to generate reference PNG; compare tank spawn animation, tank tread animation, bullet explosion, and eagle rendering against web port canvas output. Document specific tile/palette mismatches.
