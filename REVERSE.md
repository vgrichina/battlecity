...
- [ ] **Eagle wall bricks are visual-only, not in collision grid**. `drawEagleBase()` renders the Π-shaped wall (8 × 8×8 BG tiles: 4 top bar + 2 left leg + 2 right leg) but they are NOT in the `grid[]` array. Bullets pass through, tanks walk through. ROM `BrickWallInit ($C912)` writes brick tile $0F to nametable + collision map ($0400 buffer) at 8×8 resolution independently of the level map data. Fix requires: (1) add wall cells to grid+brickBits in `initLevel()` using partial-brick quadrants (row 11 cols 5-7: bottom-half bricks via brickBits 0b1000/0b1100/0b0100; row 12 cols 5,7: side-column bricks via brickBits 0b1010/0b0101); (2) let `drawTile()` + `bulletHitsTile()` handle wall rendering and destruction normally; (3) shovel power-up ($EBA0): toggle grid cells STEEL↔BRICK at wall positions; (4) remove redundant wall drawing from `drawEagleBase()`. (REVERSE.md lines 1373–1405, ROM $C912/$C9BB)
