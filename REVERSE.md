...
- [ ] **Fix render_frame.py playfield nametable offset**: `build_nametable_from_stage()` places tiles starting at nametable (row=0, col=0) = NES pixel (0,0), but spawn positions confirm the field starts at NES pixel (16,16) = nametable (row=2, col=2). Evidence: enemy spawns X=[24,120,216] Y=24 are center-pixel of metatiles 0/6/12 in row 0 — metatile-col-0 center = 16+8=24 ✓, which requires nametable col 2 = pixel 16 as field left edge. Fix: add `+2` to `tr` and `tc` in the nametable placement loop; update attribute table `aidx` calculation accordingly (ax = (mc+1)>>1, ay = (mr+1)>>1 with offset, or shift ax/ay by +1 to account for the 2-col/row border).

- [ ] **Fix extract_level_maps.py docstring water/trees swap**: Lines 14–15 say `10: trees (CHR $12)` and `11: water (CHR $22)` but ROM $DB79 dump (session 12) confirms type 10=water(CHR $12, pal1=blue) and type 11=trees(CHR $22, pal2=green). Only the comment labels are wrong; the TILE_CHR/TILE_PAL values in render_level.py and game.js are already correct.

- [ ] **Verify render_level.py partial-brick tile 0x00 skip**: For partial brick types 0–3, empty quadrants use CHR index 0x00. Code does `if tile_idx == 0x00: continue` (leaves black BG). Confirm CHR BG tile 0x00 is all-zero pixels (blank) by checking `chr_tiles[0]` — if any pixel is non-zero that quadrant should be drawn with pal[0]=black rather than skipped. Also confirm tile 0x00 is not reused as a visible tile elsewhere in the nametable.

- [ ] **Add outer border walls and eagle to render_level.py stage previews**: Current output is 13×13 metatiles only (416×416 px at 2× scale). The NES screen has a 2-tile (16px) black border on all four sides and the HUD on the right. Add a 16px border inset so the canvas matches the NES playfield region, and place the eagle base tile (DrawNametableTile type $0C/$0D = intact eagle tiles $D1–$DF, 2×2 metatile) at the bottom-center outside the 13×13 grid (metatile col 5–6, row 13).

- [ ] **Audit game.js tile rendering against render_level.py output**: Open render_level.py PNGs in Preview and open game.js in browser; compare tile-by-tile appearance for each tile type. Expected issues: (a) partial brick types 0–3 may appear as full brick in-game if game.js TILE_CHR for those types is not yet fixed to match the right-col/bot-row/left-col/top-row sub-tile pattern. (b) water/trees color may be swapped if game.js T.WATER/T.TREES enum reassignment from session 12 was not propagated to TILE_PAL lookup. (c) Attribute table palette assignment for tiles near col 0 or row 0 may differ between render_level.py and game.js due to different quad-shift formula.

- [ ] **Diagnose garbled/misaligned tile pixels — build `dump_tiles.py` ASCII viewer**: Write `dump_tiles.py` that reads the ROM CHR banks and dumps one or more tiles as ASCII art to stdout, then use it to verify the key BG terrain tiles. Script must: (a) accept `--tile N [N2 ...]` (0-based BG-bank index) and `--bank {bg,spr}` flag; (b) decode each tile as 8×8 2bpp NES pixels (plane0=bytes 0–7 LSB, plane1=bytes 8–15 MSB, bit 7=leftmost pixel); (c) render pixels as ASCII using `' ░▒█'[color_index]` (4 shades) so the tile art is visible in terminal; (d) also print the raw hex bytes for the tile so bit-level issues are diagnosable. Then run: `python dump_tiles.py --tile 0x0F 0x10 0x12 0x20 0x21 0x22` (brick/steel/water/ice/trees) and verify each looks like the expected terrain tile art from actual Battle City. If any tile looks scrambled, the 2bpp decode order or CHR bank offset (`chr_off = 16 + rom[4]*16384`) is wrong.
