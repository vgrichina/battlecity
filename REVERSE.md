...
- [ ] **Fix `game.js` CHR tile sheet path**: Update `initCHR()` to load the correct tile sheet generated from the right CHR bank. Clear tile cache. Verify all tile rendering (terrain, tanks, eagle, spawn anim, power-ups, HUD icons) uses correct graphics.
- [ ] **Fix all Python render scripts to use correct CHR bank**: Update `compare_frames.py`, `render_frame.py`, `render_level.py`, `render_sprites.py` to decode CHR tiles from the correct bank pair. Re-run comparison to verify pixel match.
- [ ] **Update compare_frames.py to produce pixel-accurate reference**: Reference should render full NES frame (BG nametable + eagle OAM sprites). gamejs_sim should match current game.js behavior exactly (border tiles, black empty tiles, eagle sprites). Diff should show only intentional differences (e.g., empty tile $00 mortar pattern in NES vs black in game.js).
